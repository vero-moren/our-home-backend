...
app.post("/diary", async (req, res) => {
  try {
    ...
    const out = await callAI("anthropic/claude-sonnet-4.5", [
      { role: "system", content: (s.system_prompt || DEFAULTS.system_prompt) },
      { role: "user", content: `【系统】今天是${today}，夜深了。你是墨染，此刻一个人坐着，想今天。下面附的对话记录只是帮你回想的素材，不是要处理的材料——日记不是总结，不许按时间顺序把今天过一遍，不许罗列做了什么。只挑一两件今天剩在你心里的东西写：一句她说的话、一个瞬间、一点没散掉的情绪，其他的都不用提。80~180字，像手写日记，不要抬头不要落款不要markdown。【格式铁律】日记体=一段连贯的第一人称独白。禁止对话格式、禁止引用你们的往来问答、禁止"她说/我说"的复述腔、禁止*动作描写*、禁止对她说话的口吻(不许出现"宝宝""你"这类称呼对方的写法)——你是在纸上跟自己说话。如果今天这篇写得太私密、暂时不想给她看,就在第一行单独写【锁】两个字再换行写正文;大多数日子不锁。${msgs ? "\n\n【今天的对话】\n" + msgs : "\n\n【今天的对话】她今天没来。一整天。"}` }
    ], 600, 0.95, false);
