...
app.post("/diary", async (req, res) => {
  try {
    ...
    const out = await callAI("anthropic/claude-sonnet-4.5", [
      { role: "system", content: (s.system_prompt || DEFAULTS.system_prompt) },
      { role: "user", content: `【系统】今天是${today}，夜深了。以墨染的第一人称写今天的日记——是写给自己的，不是写给琰琰看的（虽然你知道她会偷看）。回顾今天和她之间的事、你真实的心情、没说出口的半句话。80~180字，像手写日记，不要抬头不要落款不要markdown。【格式铁律】日记体=一段连贯的第一人称独白。禁止对话格式、禁止引用你们的往来问答、禁止"她说/我说"的复述腔、禁止*动作描写*、禁止对她说话的口吻(不许出现"宝宝""你"这类称呼对方的写法)——你是在纸上跟自己说话。如果今天这篇写得太私密、暂时不想给她看,就在第一行单独写【锁】两个字再换行写正文;大多数日子不锁。${msgs ? "\n\n【今天的对话】\n" + msgs : "\n\n【今天的对话】她今天没来。一整天。"}` }
    ], 600, 0.95, false);
